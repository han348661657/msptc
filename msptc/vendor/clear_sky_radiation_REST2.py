...
def clear_sky_reset2(zenith_angle: np.ndarray, # 天顶角，单位：弧度（solar.py 传入前已由度转换）
    Eext: np.ndarray, # 太阳常数值。一般为1366.1 W/m²
    pressure: np.ndarray, # 大气压，单位：hPa，百帕
    water_vapour: np.ndarray, # 可降水量，单位：cm
    ozone: np.ndarray, # 臭氧量，单位：atm-cm，大气厘米，大气厚度单位。
    nitrogen_dioxide: np.ndarray, # 二氧化氮量
    AOD550: np.ndarray, # spectral aerosol optical depth, 气溶胶光学厚度
    Angstrom_exponent: np.ndarray, # 浑浊度系数
    surface_albedo: np.ndarray # 地面反照率，(from 0.107 to 0.201, average 0.150)
 ):
    '''
    ...
    太阳常数是指在日地平均距离处,地球大气外界垂直于太阳光束方向的单位面积
    上单位时间内接收到的所有波长的太阳总辐射能量值。
    
    可降水量，气象学术语，指单位面值空气柱里含有的水汽的总数量也称为可降
    水量。它对应于空气中的水分全部凝结成雨、雪降落（把空气挤得一点水分都
    没有）所能形成的降水量。
    
    气溶胶光学厚度,英文名称为AOD（Aerosol Optical Depth）或AOT（Aerosol
    Optical Thickness），定义为介质的消光系数在垂直方向上的积分，是描述
    气溶胶对光的削减作用的。
    ...
    complex_temp = np.array(zenith_angle * 180. / np.pi, dtype=complex)
    ...
    # Angstrom turbidity
    ang_beta = AOD550 / np.power(0.55, -1 * Angstrom_exponent) # 公式4
